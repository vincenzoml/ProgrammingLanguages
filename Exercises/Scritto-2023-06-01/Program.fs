...
let rec findMinOfFunctionWhenRec l f flt i acc =
    match l with
    | [] -> acc
    | x::xs -> 
        let n = f (x:int)
        let m = if flt i then min n acc else acc        
        findMinOfFunctionWhenRec xs f flt (i+1) m 

let findMinOfFunctionWhen l f flt = findMinOfFunctionWhenRec l f flt 0 infinity

printfn "%A" (findMinOfFunctionWhen []  (fun x -> float x) (fun i -> i%2=0))
printfn "%A" (findMinOfFunctionWhen [1;2;3]  (fun x -> (float x)/3.0) (fun i -> i%2=0))
printfn "%A" (findMinOfFunctionWhen [1;2;3]  (fun x -> (float x)/2.0) (fun i -> i>5))
...
type Moves = Empty | Left of float * Moves | Right of float * Moves

let rec finalPos moves initialPos =
    match moves with 
    | Empty -> initialPos
    | Left (dist,next) -> finalPos next (initialPos - dist)
    | Right (dist,next) -> finalPos next (initialPos + dist)


printfn "%A" (finalPos (Left (10,(Right (5,Empty)))) -2)
...
let rec csem: com -> env -> store -> (env * store) =
    fun c ev st ->
        ...
        | CCondAssign (i,e1,e2) ->
            ...
            | Bool true ->
                match apply_env ev i with
                | L l -> let st1 = update st l s1 in (ev, st1)
                | _ -> not_a_location_error i
            | Bool false -> (ev,st)
            | _ -> type_error()
        | Cvar (i, e) ->
        ...
        | Cconst (i, e) ->
            ...
            (ev1, st)
        | CFixPoint (i,ps) ->
            let dv = apply_env ev i
            match dv with
                | L l -> 
                    let rec aux ev st v =
                        let (maxloc,stfn) = st
                        let (ev1,(maxloc1,stfn1)) = pssem ps ev st
                        let value = apply_store (maxloc1,stfn1) l
                        if value = v then 
                            (ev,st)
                        else 
                            aux ev (maxloc,stfn1) value
                    aux ev st (apply_store st l)
                | _ -> type_error()
         ...
         Pseq (
            ...
            Pseq (
                CFixPoint (
                    "x",
                    (Pseq (
                        CCondAssign ("x",Eplus (Eide "x",Eint 1),Eeql (Eide "x",Eint 0)),
                        Pseq (
                            CCondAssign ("x",Eplus (Eide "x",Eint 2),Eeql (Eide "x",Eint 1)),
                            Pend)))
                            ),
                Pend)),
         Eide "x"
