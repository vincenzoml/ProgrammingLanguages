...
let rectangleMethod (f: float -> float) (a: float) (b: float) (n: int) =
    ...
    let rec loop i sum =
        if i > n then sum
            ...
            let midpoint = a + ((float i)*dx) - (dx/2.0)
            loop (i + 1) (sum + dx * (f midpoint))
    loop 1 0.0
...
// in l, ma solo per gli elementi in cui il primo numero è maggiore di 0, e il secondo è minore di 200
...
let rec not_useful l f =
    ...
    | x::xs -> 
        ...
        let b = (f x) + 2
        if a > 0 && b < 200
        then (a,b)::(not_useful xs f) 
        else not_useful xs f
...
// 3 funzioni elementari a scelta (es: seno, coseno, somma di una specifica costante). Definire una
// funzione ricorsiva che, dato un valore x: float e una lista di FN, l= [fn1,...,fnN], calcola il valore di fnN(...(fn2(fn1(x))))

type FN = SIN | COS | PLUS3
...
let rec csem: com -> env -> store -> (env * store) =
    fun c ev st ->
        ...
        | CStrangeUntil (ps,i1,i2,e) ->
            match (ev i1,ev i2) with
            | L l1, L l2 ->
                let (maxloc,stfn0) = st
                let rec aux stfn1 =                
                    let (_,(maxloc2,stfn2)) = pssem ps ev (maxloc,stfn1)
                    let eva = esem e ev (maxloc2,stfn2)
                    let stfn3 = 
                        match eva with 
                        | Bool v -> 
                            if v then 
                                let (_,stfn3) = update (maxloc,stfn2) l1 (stfn2 l2)
                                let (_,stfn4) = update (maxloc,stfn3) l2 (stfn2 l1)
                                stfn4
                            else stfn2
                        | _ -> type_error()
                    if (stfn3 l1) = (stfn3 l2)
                    then stfn3
                    else aux stfn3
                (ev,(maxloc,aux stfn0))
            | _ -> type_error()
