...
let rec productsAux l accum =
    ...
    | x::xs -> 
        let accum' = x*accum
        accum'::(productsAux xs accum')

let products l = productsAux l 1
...
// Definire la funzione ricorsiva computeAverageBy che, data una lista lst di valori di tipo float, una funzione di filtro flt : float -> bool e una funzione fn : float -> float, calcola la media dei valori di fn(i), per i elemento della lista, considerando però solo i valori su cui la funzione flt restituisce true. Restituire 0 se non c'è almeno un valore su cui calcolare la media. 

// Per forzare un parametro ad essere di tipo float utilizzare la notazione (n: float) nel passaggio di parametri

let rec computeAverageByAux lst flt fn (accum : float,n : float) =
    match lst with
    | [] -> printfn "%A" (accum,n); if n <= 0.0 then 0.0 else accum/n
    | x::xs -> computeAverageByAux xs flt fn (if flt x then (accum+(fn (x: float)),n+1.0) else (accum,n)) 

let computeAverageBy lst flt fn = computeAverageByAux lst flt fn (0.0,0.0)
...
// forma di una coppia (x: float, y:float)), calcola la posizione di arrivo.
...
let rec csem: com -> env -> store -> (env * store) =
    fun c ev st ->
        ...
        | CSumUntil (i1,i2,e,ps) ->
            ...
            | Bool false -> (ev,st)
            | Bool true ->
                match (apply_env ev i1, apply_env ev i2) with
                | (L l1,L l2) -> 
                    let st1 = update st l1 (apply_store st l2)
                    let (_,st2) = pssem ps ev st1
                    (ev,st2)
                | _ -> type_error()
            | _ -> type_error()
